...
func NewOpenAI(cfg config.OpenAI) *OpenAI {
	...
	//   2. cfg.Key (legacy single key, only set by tests today,
	//      production callers always pass KeyPool).
	...
	if cfg.KeyPool == nil && cfg.Key != "" {
		// On error leave KeyPool nil.
		if pool, err := keypool.New([]string{cfg.Key}, quartz.NewReal()); err == nil {
			cfg.KeyPool = pool
		}
	}
	cfg.Key = ""
